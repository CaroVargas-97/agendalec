    ...
    <div style={{ display: "flex", flex: 1, flexDirection: "column" }}>
      <div style={{ ...s.main, padding: isMobile ? "1rem" : "1.5rem" }}>
                  ...
                  {turnos.some(t => !t.start_time) && (
                    <div style={{ marginBottom: "10px", display: "flex", flexDirection: "column", gap: "6px" }}>
                      ...
                      {turnos.filter(t => !t.start_time).map((t, i) => {
                        ...
                        return (
                          <div key={i} onClick={() => abrirTurno(t)} style={{ padding: "8px 12px", borderRadius: "8px", background: isLimpieza ? "#EAF6EE" : isPartial ? "#EDE8FA" : isPending ? "#FFFBEB" : "#F8F4FC", borderLeft: `4px solid ${isLimpieza ? "#3B8C5A" : isPartial ? "#5C3F99" : isPending ? "#D97706" : "#9B72C0"}`, cursor: "pointer", fontSize: "12px", color: "#2A1845" }}>
                            {isLimpieza && "🌿 "}{isPartial && "💜 "}<strong>{t.clients?.full_name}</strong> · {t.services?.name}{es2x1(t) && " · 🎁 2x1"}
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "52px 1fr" }}>
                    ...
                    <div style={{ position: "relative", borderLeft: "0.5px solid #F0E8F8", height: `${HORAS.length * 64}px` }}>
                      ...
                      {turnos.filter(t => t.start_time && t.status !== "cancelled").map((t, i) => {
                        ...
                        const accent = isCancelled ? "#9CA3AF" : isLimpieza ? "#3B8C5A" : isPartial ? "#5C3F99" : isPending ? "#D97706" : isVirtual ? "#7C3AED" : "#BE185D";
                        const bg = isCancelled ? "#F3F4F6" : isLimpieza ? "#EAF6EE" : isPartial ? "#EDE8FA" : isPending ? "#FFFBEB" : isVirtual ? "#F3E8FF" : "#FDF2F8";
                        const textColor = isCancelled ? "#6B7280" : isLimpieza ? "#1F5C37" : isPartial ? "#3B2460" : isPending ? "#78350F" : isVirtual ? "#4C1D95" : "#831843";
                        const label = isCancelled ? "Cancelado" : isLimpieza ? "Limpieza" : isPartial ? "Seña pagada" : isPending ? "Pendiente" : isVirtual ? "Virtual" : "Presencial";
                          ...
                          {esBloqueado ? null : (
                            <div>
                              ...
                              {turnosDelDia.length === 0 ? (
                                ...
                                const isPending = t.status === "pending";
                                const isVirtual = t.modality === "virtual";
                                const accent = isLimpieza ? "#3B8C5A" : isPartial ? "#5C3F99" : isPending ? "#D97706" : isVirtual ? "#7C3AED" : "#BE185D";
                                const bg = isLimpieza ? "#EAF6EE" : isPartial ? "#EDE8FA" : isPending ? "#FFFBEB" : isVirtual ? "#F3E8FF" : "#FDF2F8";
                                const textColor = isLimpieza ? "#1F5C37" : isPartial ? "#3B2460" : isPending ? "#78350F" : isVirtual ? "#4C1D95" : "#831843";
                                const emoji = isLimpieza ? "🌿" : isPartial ? "💜" : isPending ? "⏳" : isVirtual ? "📹" : "📍";
                                return (
                  ...
                  {turnosSemana.some(t => !t.start_time && t.status !== "cancelled") && (
                    <div style={{ marginBottom: "10px", display: "flex", flexDirection: "column", gap: "6px" }}>
                      ...
                      {turnosSemana.filter(t => !t.start_time && t.status !== "cancelled").map((t, i) => {
                        ...
                        return (
                          <div key={i} onClick={() => abrirTurno(t)} style={{ padding: "8px 12px", borderRadius: "8px", background: isLimpieza ? "#EAF6EE" : isPartial ? "#EDE8FA" : isPending ? "#FFFBEB" : "#F8F4FC", borderLeft: `4px solid ${isLimpieza ? "#3B8C5A" : isPartial ? "#5C3F99" : isPending ? "#D97706" : "#9B72C0"}`, cursor: "pointer", fontSize: "12px", color: "#2A1845" }}>
                            {isLimpieza && "🌿 "}{isPartial && "💜 "}<strong>{t.clients?.full_name}</strong> · {t.services?.name}{es2x1(t) && " · 🎁 2x1"}{t.date ? ` · ${new Date(t.date + "T12:00:00").toLocaleDateString("es-AR", { weekday: "short", day: "numeric" })}` : ""}
                          </div>
                        );
                      })}
                    </div>
        ...
        <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
          {[["#F3E8FF","#7C3AED","Virtual"],["#FDE8F0","#E88BB0","Presencial"],["#FFF8E8","#F0A800","Pendiente"],["#EDE8FA","#5C3F99","💜 Seña pagada"]].map(([bg,border,label]) => (
